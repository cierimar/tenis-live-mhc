...
if ($players.Count -eq 0 -and (Test-Path $outFile)) {
    try {
        $prev = [System.IO.File]::ReadAllText($outFile) | ConvertFrom-Json
        if ($prev.players -and @($prev.players).Count -gt 0) {
            Write-Host "scrape vino vacio; se conserva birthdays.json previo con $(@($prev.players).Count) jugadores"
            exit 0
        }
    } catch {}
}
